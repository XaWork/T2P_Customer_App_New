...
class GharKaKhanaAddToCartUseCase @Inject constructor(
    ...
    suspend fun execute(
        ...
        subCategory: String,
        weight: String,
    ): Flow<Resource<GharKaKhanaAddToCartModel>> {
        return flow {
            emit(Resource.Loading(true))
            try {
                val userId = userPref.getUser()!!.id
                val response =
                    repo.gharKaKhanaAddToCart(userId, product, category, subCategory, weight)
