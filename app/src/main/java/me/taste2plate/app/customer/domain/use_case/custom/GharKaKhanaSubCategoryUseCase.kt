...
class GharKaKhanaSubCategoryUseCase @Inject constructor(
    private val repo: CustomRepo,
) {
    suspend fun execute(): Flow<Resource<GharKaKhanaSubCategoryModel>> {
        return flow {
            emit(Resource.Loading(true))
            try {
                val response = repo.gharKaKhanaSubCategory()
